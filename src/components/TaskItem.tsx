    ...
    >
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex flex-col items-end gap-2">
          {/* Task Status Chip */}
          <span className="rounded-full bg-primary px-2 py-1 text-[10px] text-muted">
            {task.status}
          </span>

          {/* Action Buttons: Expend on click */}
          <div
            className={`flex gap-2 overflow-hidden transition-all duration-200 ease-out ${extendCard ? "max-h-12 opacity-100" : "max-h-0 opacity-0"}`}
          >
            {/* Delete Task */}
            <button
              onClick={(e) => {
